...
export class CommentService implements ICommentService {
  ...
  async create(data: CreateCommentRequestData): Promise<void> {
    try {
      ...
      const outputDTO = new CreateCommentOutputDTO()
      const resData = new CreateCommentResponseData(400, error.message, outputDTO)
      await this._commentPresenter.createCommentPresenter(resData)
      return
    }
    try {
  ...
  async getListCommentByReviewId(data: GetListCommentByReviewIdRequestData): Promise<void> {
    ...
    try {
      ...
      if (!review) {
        ...
        const resData = new GetListCommentByReviewIdResponseData(400, "Review not found", outputDTO)
      ...
      const outputDTO = new GetListCommentByReviewIdOutputDTO([])
      const resData = new GetListCommentByReviewIdResponseData(400, err.message, outputDTO)
      await this._commentPresenter.getListCommentByReviewIdPresenter(resData)
      return
    }
  ...
  async update(data: UpdateCommentRequestData): Promise<void> {
    ...
    // Kiem tra User
    try{
        const response = this._commentDatabase.findUser(userId)
        if(!response){
            const dto = new UpdateCommentOutputDTO()
            const resData = new UpdateCommentResponseData(400, "User not found", dto)
            await this._commentPresenter.updateCommentPresenter(resData)
            return
        }
    }catch(err){
        const dto = new UpdateCommentOutputDTO()
        const resData = new UpdateCommentResponseData(400, err.message, dto)
        await this._commentPresenter.updateCommentPresenter(resData)
        return
    }
    ...
    // Kiem tra bai review
    try{
        const resposne = this._commentDatabase.findReview(reviewId)
        if(!resposne){
            const dto = new UpdateCommentOutputDTO()
            const resData = new UpdateCommentResponseData(400, "Review not found", dto)
            await this._commentPresenter.updateCommentPresenter(resData)
            return
        }
    }catch(err){
    ...
    try {
      ...
      if (!comment) {
        const dto = new UpdateCommentOutputDTO()
        const resData = new UpdateCommentResponseData(400, "Comment not found", dto)
        await this._commentPresenter.updateCommentPresenter(resData)
        return
      }
    } catch (error) {
      const dto = new UpdateCommentOutputDTO()
      const resData = new UpdateCommentResponseData(400, error.message, dto)
      await this._commentPresenter.updateCommentPresenter(resData)
      return
    }
